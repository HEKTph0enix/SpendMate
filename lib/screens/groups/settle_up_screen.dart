    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (users.isNotEmpty) {
        setState(() {
          ...
          _paidBy = users.firstWhere((u) => u.id == settings.currentUserId,
              orElse: () => users.first);

          // Default paid to is someone else
          _paidTo = users.firstWhere((u) => u.id != _paidBy?.id,
              orElse: () => users.last);
        });
      }
    });
  ...
  void _saveSettlement() async {
    ...
    if (_paidBy?.id == _paidTo?.id) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Payer and Payee cannot be the same person.')),
      );
      return;
    }
